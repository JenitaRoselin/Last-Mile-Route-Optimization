...
export default function VoiceBotFAB() {
  ...
  const handleCommand = (text: string) => {
    ...
    console.log("🎤 Voice Bot heard:", lowerText);

    if (lowerText.includes('map') || lowerText.includes('kaatu') || lowerText.includes('route') || lowerText.includes('vali')) {
      router.push('/map');
    } 
    else if (lowerText.includes('fuel') || lowerText.includes('petrol') || lowerText.includes('diesel') || lowerText.includes('bunk')) {
      router.push('/fuel-log');
    } 
    else if (lowerText.includes('next') || lowerText.includes('delivery') || lowerText.includes('adutha') || lowerText.includes('order')) {
      router.push('/urgent-order');
    } 
    else if (lowerText.includes('break') || lowerText.includes('rest') || lowerText.includes('stop')) {
      router.push('/profile');
    }
  };
  ...
  const startListening = () => {
    if (Platform.OS !== 'web') {
      alert("Voice demo is only for Web right now.");
      return;
    }
    ...
    if (!SpeechRecognition) {
      alert("Please use Google Chrome or Edge for the voice demo.");
      return;
    }
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity 
        style={[styles.fab, isListening && styles.listeningActive]} 
        onPress={startListening}
        onLongPress={() => router.push('/map')} // Fail-safe: Long press to force navigation
      >
        <Text style={styles.micText}>{isListening ? "Listening..." : "🎤 Mic"}</Text>
      </TouchableOpacity>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  listeningActive: {
    backgroundColor: '#ef4444', // Turns red when listening
  },
  micText: {
    color: 'white',
    fontSize: 12,
